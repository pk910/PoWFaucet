...
describe("Faucet Process", () => {
  ...
  it("Check process event handler: uncaughtException", async () => {
    ...
    globalStubs["FaucetProcess.emitLog"] = sinon.stub(
      FaucetProcess.prototype,
      "emitLog"
    );
    setTimeout(function () {
    ...
    expect(globalStubs["process.exit"].callCount).to.equal(
      1,
      "process.exit not called"
    );
    expect(globalStubs["FaucetProcess.emitLog"].getCall(0).args[0]).to.equal(
      "ERROR",
      "missing log entry"
    );
    expect(globalStubs["FaucetProcess.emitLog"].getCall(0).args[1]).to.match(
      /unhandled exception/,
      "missing log entry"
    );
  });
  ...
  it("Check logging: stdout", async () => {
    ...
    expect(globalStubs["console.log"].getCall(0).args[0]).to.match(
      /test log message/,
  ...
  it("Check logging: file", async () => {
    ...
    await faucetProcess.initialize();
    faucetProcess.emitLog(FaucetLogLevel.INFO, "test log message");
    expect(fs.existsSync(faucetConfig.faucetLogFile)).to.equal(
      true,
      "log file not found"
    );
  ...
  it("Check pid file", async () => {
    ...
    let faucetProcess = ServiceManager.GetService(FaucetProcess);
    await faucetProcess.initialize();
    expect(fs.existsSync(faucetConfig.faucetPidFile)).to.equal(
      true,
      "pid file not found"
    );
