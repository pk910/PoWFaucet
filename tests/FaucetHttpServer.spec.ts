...
import {
  FaucetHttpResponse,
  FaucetHttpServer,
} from "../src/webserv/FaucetHttpServer.js";
...
describe("Faucet Web Server", () => {
  ...
  it("check api call (POST)", async () => {
    ...
    let responseData = await fetch(
      "http://localhost:" + listenPort + "/api/testEndpoint",
      {
        method: "POST",
    ...
    expect(responseData).equals(
      '"1da06016289bd76a5ada4f52fc805ae0c394612f17ec6d0f0c29b636473c8a9d"',
  ...
  it("check api call (POST, body size limit)", async () => {
    ...
    expect(error.toString()).to.matches(
      /socket hang up/,
  ...
  it("check ws call", async () => {
    ...
    webSocket.onmessage = (evt) => {
      ...
      if (data && data.action === "error") errorDfd.resolve(data);
    };
    ...
    expect(errorResponse.data.reason).to.matches(
      /session not found/,
