...
export function bindTestStubs(stubs?) {
  ...
  let allStubs = {
    _state: stateDict,
    "FaucetWorkers.createWorker": sinon
      .stub(FaucetWorkers.prototype, "createWorker")
      .callsFake((classKey) => {
...
export function fusedSleep(fuseFn: any, timeout?: number): Promise<void> {
  return new Promise((resolve, reject) => {
    if (fuseFn?._dfd) fuseFn._dfd.promise.then(resolve, reject);
    if (timeout) sleepPromise(timeout).then(resolve);
  });
}

export function returnDelayedPromise(
  resolve: boolean,
  result: any,
  delay?: number
): Promise<any> {
