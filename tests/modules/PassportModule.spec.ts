...
describe("Faucet module: passport", () => {
  ...
  it("Check client config exports", async () => {
    ...
    expect(!!clientConfig.modules["passport"]).to.equal(
      true,
      "no passport config exported"
    );
    expect(clientConfig.modules["passport"].refreshTimeout).to.equal(
      30,
      "client config missmatch: refreshTimeout"
    );
    expect(clientConfig.modules["passport"].manualVerification).to.equal(
      true,
      "client config missmatch: manualVerification"
    );
    expect(
      JSON.stringify(clientConfig.modules["passport"].stampScoring)
    ).to.equal(
      JSON.stringify((faucetConfig.modules["passport"] as any).stampScoring),
      "client config missmatch: stampScoring"
    );
    expect(
      JSON.stringify(clientConfig.modules["passport"].boostFactor)
    ).to.equal(
      JSON.stringify((faucetConfig.modules["passport"] as any).boostFactor),
      "client config missmatch: boostFactor"
    );
  ...
  it("Start session with successful passport request", async () => {
    ...
    globalStubs["PassportResolver.getVerifyTime"] = sinon
      .stub(PassportResolver.prototype as any, "getVerifyTime")
      .returns(1686844923);
    await ServiceManager.GetService(ModuleManager).initialize();
    ...
    expect(!!(clientInfo.modules as any)["passport"]).to.equal(
      false,
      "unexpected passport info in client session info"
    );
  });
  ...
  it("Start session with passport api error", async () => {
    ...
    let passportData = testSession.getSessionData("passport.data");
    expect(passportData?.found).to.equal(
      false,
      "unexpected passport data found"
    );
    let passportScore = testSession.getSessionData("passport.score");
  ...
  it("Send passport info to client for running sessions", async () => {
    ...
    expect(!!(clientInfo.modules as any)["passport"]).to.equal(
      true,
      "missing passport info in client session info"
    );
    expect((clientInfo.modules as any)["passport"].score).to.equal(
      2,
      "unexpected passport score"
    );
    expect((clientInfo.modules as any)["passport"].factor).to.equal(
      4,
      "unexpected passport factor"
    );
  });
  ...
  it("Get passport details for running session", async () => {
    ...
    let passportDetailsRsp = await ServiceManager.GetService(
      FaucetWebApi
    ).onApiRequest(
      {
        method: "GET",
    ...
    expect(passportDetailsRsp instanceof FaucetHttpResponse).to.equal(
      false,
      "unexpected plain http response"
    );
    expect(passportDetailsRsp.passport?.found).to.equal(
      true,
      "no passport details returned"
    );
    expect(passportDetailsRsp.score.score).to.equal(
      2,
      "unexpected passport score"
    );
    expect(passportDetailsRsp.score.factor).to.equal(
      4,
      "unexpected passport factor"
    );
  });
  ...
  it("Get passport details for unknown session", async () => {
    ...
    expect(passportDetailsRsp.code).to.equal(
      "INVALID_SESSION",
  ...
  it("Check passport cache (DB cache)", async () => {
    ...
    expect(testSession.getSessionStatus()).to.equal(
      "claimable",
      "unexpected session1 status"
    );
    expect(globalStubs["fetch"].callCount).to.equal(
      1,
      "unexpected fetch call count after session1 start"
    );
    ...
    expect(testSession.getSessionStatus()).to.equal(
      "claimable",
      "unexpected session2 status"
    );
    expect(globalStubs["fetch"].callCount).to.equal(
      1,
      "unexpected fetch call count after session2 start"
    );
    let passportData = testSession.getSessionData("passport.data");
    expect(passportData?.found).to.equal(true, "no passport data found");
    let passportScore = testSession.getSessionData("passport.score");
  ...
  it("Check passport cache (DB cache, race condition)", async () => {
    ...
    expect(globalStubs["fetch"].callCount).to.equal(
      1,
      "unexpected fetch call count after session start"
    );
    let passportData = testSession.getSessionData("passport.data");
    expect(passportData?.found).to.equal(true, "no passport data found");
    let passportScore = testSession.getSessionData("passport.score");
  ...
  it("Refresh passport for a running session (automatic refresh)", async () => {
    ...
    globalStubs["PassportResolver.getVerifyTime"] = sinon
      .stub(PassportResolver.prototype as any, "getVerifyTime")
      .returns(1686844923);
    let passportRefreshRsp = await ServiceManager.GetService(
      FaucetWebApi
    ).onApiRequest(
      {
        method: "GET",
    ...
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
      "unexpected plain http response"
    );
    expect(passportRefreshRsp.passport?.found).to.equal(
      true,
      "no passport details returned in refresh result"
    );
    expect(passportRefreshRsp.score.score).to.equal(
      2,
      "unexpected passport score in refresh result"
    );
    expect(passportRefreshRsp.score.factor).to.equal(
      4,
      "unexpected passport factor in refresh result"
    );
    let now = Math.floor(new Date().getTime() / 1000);
    expect(Math.abs(passportRefreshRsp.cooldown - now)).to.be.lessThan(
      2,
      "unexpected cooldown"
    );
    ...
    expect(passportData?.found).to.equal(
      true,
      "no passport data found after refresh"
    );
    let passportScore = testSession.getSessionData("passport.score");
    expect(passportScore?.score).to.equal(2, "unexpected passport score");
    expect(passportScore?.factor).to.equal(4, "unexpected passport factor");
  ...
  it("Refresh passport for a running session (manual refresh)", async () => {
    ...
    let passportRefreshRsp = await ServiceManager.GetService(
      ...
      Buffer.from(
        JSON.stringify(JSON.stringify((testData as any).testPassport1Json))
      )
    );
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
  ...
  it("Refresh passport for a running session (manual refresh, invalid json)", async () => {
    ...
    let passportRefreshRsp = await ServiceManager.GetService(
      ...
      Buffer.from(
        JSON.stringify(JSON.stringify({ not: "a_passport", json: 1 }))
      )
    );
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
    ...
    expect(passportRefreshRsp.code).to.equal(
      "PASSPORT_VALIDATION",
  ...
  it("Refresh passport for a running session (manual refresh, invalid json 1)", async () => {
    ...
    let passportRefreshRsp = await ServiceManager.GetService(
      ...
      Buffer.from(
        JSON.stringify(JSON.stringify((testData as any).invalidPassportJson1))
      )
    );
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
    ...
    expect(passportRefreshRsp.errors.length).to.equal(
      1,
      "unexpected number of verification errors returned"
    );
  });
  it("Refresh passport for a running session (manual refresh, invalid json 2)", async () => {
    ...
    let passportRefreshRsp = await ServiceManager.GetService(
      ...
      Buffer.from(
        JSON.stringify(JSON.stringify((testData as any).invalidPassportJson2))
      )
    );
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
    ...
    expect(passportRefreshRsp.errors[0]).to.match(
      /duplicate provider/,
      "unexpected verification error returned"
    );
  });
  it("Refresh passport for a running session (manual refresh, invalid json 3)", async () => {
    ...
    let passportRefreshRsp = await ServiceManager.GetService(
      ...
      Buffer.from(
        JSON.stringify(JSON.stringify((testData as any).invalidPassportJson3))
      )
    );
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
    ...
    expect(passportRefreshRsp.errors[0]).to.match(
      /not signed for expected wallet/,
      "unexpected verification error returned"
    );
  });
  it("Refresh passport for a running session (manual refresh, invalid json 4)", async () => {
    ...
    let passportRefreshRsp = await ServiceManager.GetService(
      ...
      Buffer.from(
        JSON.stringify(JSON.stringify((testData as any).invalidPassportJson4))
      )
    );
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
    ...
    expect(passportRefreshRsp.errors[1]).to.match(
      /invalid proof verificationMethod/,
      "unexpected verification error returned"
    );
  });
  it("Refresh passport for a running session (manual refresh, invalid json 5)", async () => {
    ...
    let passportRefreshRsp = await ServiceManager.GetService(
      ...
      Buffer.from(
        JSON.stringify(JSON.stringify((testData as any).invalidPassportJson5))
      )
    );
    expect(passportRefreshRsp instanceof FaucetHttpResponse).to.equal(
      false,
    ...
    expect(passportRefreshRsp.errors[2]).to.match(
      /integrity check failed/,
      "unexpected verification error returned"
    );
  });
