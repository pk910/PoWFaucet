...
describe("Faucet module: github", () => {
  ...
  function fakeFetch(url: any, opts: any): Promise<any> {
    for (let i = 0; i < fakeFetchResponses.length; i++) {
      if (fakeFetchResponses[i].url.test(url)) {
        ...
        if (!fakeFetchResponses[i].calls) fakeFetchResponses[i].calls = [];
        fakeFetchResponses[i].calls.push({
          url: url,
          opts: opts,
        });
        return promise.then(() => {
          if (fakeFetchResponses[i].fail) throw fakeFetchResponses[i].rsp;
  ...
  it("Check client config exports", async () => {
    ...
    expect(!!clientConfig.modules["github"]).to.equal(
      true,
      "no github config exported"
    );
    expect(clientConfig.modules["github"].clientId).to.equal(
      "test-client-id",
      "client config missmatch: clientId"
    );
    expect(clientConfig.modules["github"].authTimeout).to.equal(
      86400,
      "client config missmatch: authTimeout"
    );
    expect(clientConfig.modules["github"].redirectUrl).to.equal(
      "test-redirect-url",
      "client config missmatch: redirectUrl"
    );
    expect(clientConfig.modules["github"].callbackState).to.equal(
      "test-callback-state",
      "client config missmatch: callbackState"
    );
  });
  ...
  it("Start session with github token", async () => {
    ...
    expect(apiRsp.userInfo.calls?.length || 0).to.equal(
      1,
      "unexpected number of user info api calls"
    );
    expect(apiRsp.userInfo.calls[0].opts?.headers["Authorization"]).to.equal(
      "token test-github-token",
      "unexpected access token in api call"
    );
    expect(apiRsp.ownRepos.calls?.length || 0).to.equal(
      0,
      "unexpected number of own repos api calls"
    );
    expect(testSession.getSessionData("github.uid")).to.equal(
      1337,
      "unexpected github info in session data: github.uid"
    );
    expect(testSession.getSessionData("github.user")).to.equal(
      "testus",
      "unexpected github info in session data: github.user"
    );
  });
  ...
  it("Check github requirements: unexpected api error", async () => {
    ...
    try {
      await ServiceManager.GetService(SessionManager).createSession(
        "::ffff:8.8.8.8",
        {
          addr: "0x0000000000000000000000000000000000001337",
          githubToken: generateTestToken("test-github-token", 1337),
  ...
  it("Check github requirements: own repository count", async () => {
    ...
    expect(error?.getCode()).to.equal("GITHUB_CHECK", "unexpected error code");
    expect(error?.message).to.matches(
      /own repository count check failed/,
      "unexpected error message"
    );
  ...
  it("Check github requirements: own repository stars", async () => {
    ...
    expect(error?.getCode()).to.equal("GITHUB_CHECK", "unexpected error code");
    expect(error?.message).to.matches(
      /own repository star count check failed/,
      "unexpected error message"
    );
  ...
  it("Check reward factor handling", async () => {
    ...
    expect(testSession.getSessionData("github.factor")).to.equal(
      2,
      "unexpected github info in session data: github.factor"
    );
  ...
  it("Check authentication callback: authentication error", async () => {
    await ServiceManager.GetService(ModuleManager).initialize();
    let callbackRsp = await ServiceManager.GetService(
      FaucetWebApi
    ).onApiRequest({
      method: "GET",
      url: "/api/githubCallback?error=test_error&error_description=test+error+message",
    } as any);
    expect(callbackRsp instanceof FaucetHttpResponse).to.equal(
      true,
      "unexpected api response"
    );
    expect(callbackRsp.body).to.matches(
      /github\.AuthResult/,
      "unexpected response page"
    );
    expect(callbackRsp.body).to.matches(
      /test_error/,
      "test error code not in response page"
    );
  });
  ...
  it("Check authentication callback: unknown parameters", async () => {
    ...
    expect(callbackRsp.body).to.matches(
      /UNKNOWN/,
      "UNKNOWN error code not in response page"
    );
  });
  ...
  it("Check authentication callback: successful authentication", async () => {
    ...
    expect(callbackRsp.body).to.matches(
      /"user": *"testus"/,
      "auth result not in response page"
    );
  });
  ...
  it("Check authentication callback: api error", async () => {
    ...
    expect(callbackRsp.body).to.matches(
      /AUTH_ERROR/,
      "AUTH_ERROR error code not in response page"
    );
  });
  ...
  it("Check authentication callback: unexpected error", async () => {
    await ServiceManager.GetService(ModuleManager).initialize();
    let apiRsp = addGithubApiResponses();
    apiRsp.authToken.rsp = {
      text: () => Promise.resolve("no proper response"),
    };
    let callbackRsp = await ServiceManager.GetService(
      FaucetWebApi
    ).onApiRequest({
  ...
  it("Check github based recurring restrictions: exceed session count", async () => {
    ...
    expect(testSession1.getSessionData("github.uid")).to.equal(
      1337,
      "unexpected github info in session data: github.uid"
    );
