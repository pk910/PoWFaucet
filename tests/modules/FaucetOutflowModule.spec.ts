...
describe("Faucet module: faucet-outflow", () => {
  ...
  it("Start sessions with decreasing drop amount", async () => {
    ...
    expect(await runTestSession()).to.equal(
      81n,
      "unexpected drop amount: session 3"
    );
    await sleepPromise(1000);
    expect((await runTestSession()) <= 82n).to.equal(
      true,
      "unexpected drop amount: session 4"
    );
  }).timeout(3000);
  ...
  it("Check outflow balance overflow", async () => {
    ...
    await moduleManager.initialize();
    let outflowModule =
      moduleManager.getModule<FaucetOutflowModule>("faucet-outflow");
    (outflowModule as any).outflowState.trackTime -= 20;
    expect(outflowModule.getOutflowDebugState().balance).to.equal(
      "1000",
      "unexpected outflow balance after 0 sessions"
    );
    for (let i = 0; i < 11; i++) {
      expect(await runTestSession()).to.equal(
        100n,
        "unexpected drop amount: session " + (i + 1)
      );
      expect(outflowModule.getOutflowDebugState().balance).to.equal(
        (1000 - (i + 1) * 100).toString(),
        "unexpected outflow balance after " + (i + 1) + " sessions"
      );
    }
    expect(await runTestSession()).to.equal(
      90n,
      "unexpected drop amount: session 12"
    );
  }).timeout(3000);
