...
describe("Faucet module: pow", () => {
  ...
  it("Check client config exports (scrypt)", async () => {
    ...
    let clientConfig =
      ServiceManager.GetService(FaucetWebApi).onGetFaucetConfig();
    expect(!!clientConfig.modules["pow"]).to.equal(
      true,
      "no pow config exported"
    );
    expect(clientConfig.modules["pow"].powTimeout).to.equal(
      60,
      "client config missmatch: powTimeout"
    );
    expect(clientConfig.modules["pow"].powParams.a).to.equal(
      PoWHashAlgo.SCRYPT,
      "client config missmatch: powParams.a"
    );
    expect(clientConfig.modules["pow"].powParams.n).to.equal(
      4096,
      "client config missmatch: powParams.n"
    );
    expect(clientConfig.modules["pow"].powParams.r).to.equal(
      8,
      "client config missmatch: powParams.r"
    );
    expect(clientConfig.modules["pow"].powParams.p).to.equal(
      1,
      "client config missmatch: powParams.p"
    );
    expect(clientConfig.modules["pow"].powParams.l).to.equal(
      16,
      "client config missmatch: powParams.l"
    );
    expect(clientConfig.modules["pow"].powDifficulty).to.equal(
      11,
      "client config missmatch: powDifficulty"
    );
    expect(clientConfig.modules["pow"].powNonceCount).to.equal(
      2,
      "client config missmatch: powNonceCount"
    );
    expect(clientConfig.modules["pow"].powHashrateLimit).to.equal(
      1337,
      "client config missmatch: powHashrateLimit"
    );
    let powModule = moduleManager.getModule<PoWModule>("pow");
    expect(powModule.getPoWParamsStr()).to.equal(
      "scrypt|4096|8|1|16|11",
      "invalid powParams string"
    );
  });
  ...
  it("Check client config exports (cryptonight)", async () => {
    ...
    expect(clientConfig.modules["pow"].powParams.a).to.equal(
      PoWHashAlgo.CRYPTONIGHT,
      "client config missmatch: powParams.a"
    );
    expect(clientConfig.modules["pow"].powParams.c).to.equal(
      0,
      "client config missmatch: powParams.c"
    );
    expect(clientConfig.modules["pow"].powParams.v).to.equal(
      1,
      "client config missmatch: powParams.v"
    );
    expect(clientConfig.modules["pow"].powParams.h).to.equal(
      10,
      "client config missmatch: powParams.h"
    );
    expect(clientConfig.modules["pow"].powDifficulty).to.equal(
      11,
      "client config missmatch: powDifficulty"
  ...
  it("Check client config exports (argon2)", async () => {
    ...
    expect(clientConfig.modules["pow"].powParams.t).to.equal(
      0,
      "client config missmatch: powParams.t"
    );
    expect(clientConfig.modules["pow"].powParams.v).to.equal(
      13,
      "client config missmatch: powParams.v"
    );
    expect(clientConfig.modules["pow"].powParams.i).to.equal(
      4,
      "client config missmatch: powParams.i"
    );
    expect(clientConfig.modules["pow"].powParams.m).to.equal(
      4096,
      "client config missmatch: powParams.m"
    );
    expect(clientConfig.modules["pow"].powParams.p).to.equal(
      1,
      "client config missmatch: powParams.p"
  ...
  it("Start mining session and check session params", async () => {
    ...
    expect(!!(clientInfo.modules as any)["pow"]).to.equal(
      true,
      "missing pow info in client session info"
    );
    expect((clientInfo.modules as any)["pow"].lastNonce).to.equal(
      0,
      "invalid pow info in client session info: lastNonce"
    );
    expect((clientInfo.modules as any)["pow"].preImage).to.equal(
      testSession.getSessionData("pow.preimage"),
      "invalid pow info in client session info: preImage"
    );
    expect((clientInfo.modules as any)["pow"].shareCount).to.equal(
      0,
      "invalid pow info in client session info: shareCount"
    );
  });
  ...
  it("Start mining session and connect mining client", async () => {
    ...
    let fakeWs = await injectFakeWebSocket(
      "/ws/pow?session=" + testSession.getSessionId(),
      "8.8.8.8"
    );
    expect(fakeWs.isReady).to.equal(true, "websocket was closed");
    let errorMsg = fakeWs.getSentMessage("error");
    expect(errorMsg.length).to.equal(
      0,
      "a unexpected error message has been sent: " +
        (errorMsg.length ? errorMsg[0].data.code : "")
    );
  });
  ...
  it("Connect invalid mining client (missing session id)", async () => {
    ...
    expect(errorMsg[0].data.code).to.equal(
      "INVALID_SESSION",
  ...
  it("Connect invalid mining client (unknown session id)", async () => {
    ...
    await moduleManager.initialize();
    let fakeWs = await injectFakeWebSocket(
      "/ws/pow?session=e36ec5e6-12ee-4015-951f-b018b37de451",
      "8.8.8.8"
    );
    expect(fakeWs.isReady).to.equal(false, "websocket not closed");
    let errorMsg = fakeWs.getSentMessage("error");
  ...
  it("Connect multiple mining clients for same session", async () => {
    ...
    let moduleManager = ServiceManager.GetService(ModuleManager);
    await moduleManager.initialize();
    let testSession = await ServiceManager.GetService(
      SessionManager
    ).createSession("::ffff:8.8.8.8", {
      addr: "0x0000000000000000000000000000000000001337",
    });
    expect(testSession.getSessionStatus()).to.equal(
      "running",
  ...
  describe("Mining websocket protocol", () => {
    it("check ping timeout handling", async () => {
      ...
      globalStubs["FakeWebSocket.ping"] = sinon.stub(
        FakeWebSocket.prototype,
        "ping"
      );
      globalStubs["FakeWebSocket.pong"] = sinon.stub(
        FakeWebSocket.prototype,
        "pong"
      );
      let moduleManager = ServiceManager.GetService(ModuleManager);
      ...
      let fakeSocket = await injectFakeWebSocket(
        "/ws/pow?session=" + testSession.getSessionId(),
        "8.8.8.8"
      );
      fakeSocket.emit("pong");
      fakeSocket.emit("ping");
      expect(globalStubs["FakeWebSocket.pong"].called).to.equal(
        true,
        "pong not called"
      );
      expect(globalStubs["FakeWebSocket.ping"].called).to.equal(
        false,
        "unexpected ping call"
      );
      await awaitSleepPromise(
        1100,
        () => globalStubs["FakeWebSocket.ping"].called
      );
    ...
    it("check action 'foundShare': invalid share data", async () => {
      ...
      expect(errorMsg[0].data.message).to.equal(
        "Invalid share data",
    ...
    it("check action 'foundShare': invalid nonce count", async () => {
      ...
      expect(errorMsg[0].data.message).to.matches(
        /Invalid nonce count/i,
        "unexpected error message"
      );
    });
    ...
    it("check action 'foundShare': valid share, local verification, scrypt", async () => {
      ...
      await awaitSleepPromise(
        1000,
        () => fakeSocket.getSentMessage("ok").length > 0
      );
      ...
      await awaitSleepPromise(
        100,
        () => fakeSocket.getSentMessage("updateBalance").length > 0
      );
      let balanceMsg = fakeSocket.getSentMessage("updateBalance");
      expect(balanceMsg.length).to.equal(1, "no updateBalance message sent");
      expect(balanceMsg[0].data.balance).to.equal(
        "10",
        "invalid updateBalance message: unexpected balance"
      );
      expect(balanceMsg[0].data.reason).to.matches(
        /valid share/,
        "invalid updateBalance message: unexpected reason"
      );
    });
    ...
    it("check action 'foundShare': invalid share, local verification, scrypt", async () => {
      ...
      expect(errorMsg.length).to.equal(
        2,
        "unexpected number of error messages sent"
      );
      expect(errorMsg[0].rsp).to.equal(42, "invalid response id");
      expect(errorMsg[0].data.code).to.equal(
        "WRONG_SHARE",
        "unexpected error1 code"
      );
      expect(errorMsg[0].data.message).to.matches(
        /verification failed/i,
        "unexpected error1 message"
      );
      expect(errorMsg[1].data.code).to.equal(
        "CLIENT_KILLED",
        "unexpected error2 code"
      );
      expect(errorMsg[1].data.message).to.matches(
        /session failed/i,
        "unexpected error2 message"
      );
    ...
    it("check action 'verifyResult': valid share verification", async () => {
      ...
      let moduleManager = ServiceManager.GetService(ModuleManager);
      await moduleManager.initialize();
      let testSession1 = await ServiceManager.GetService(
        SessionManager
      ).createSession("::ffff:8.8.8.8", {
        addr: "0x0000000000000000000000000000000000001337",
      });
      let fakeSocket1 = await injectFakeWebSocket(
        "/ws/pow?session=" + testSession1.getSessionId(),
        "8.8.8.8"
      );
      ...
      let testSession2 = await ServiceManager.GetService(
        SessionManager
      ).createSession("::ffff:8.8.4.4", {
        addr: "0x0000000000000000000000000000000000001338",
      });
      let fakeSocket2 = await injectFakeWebSocket(
        "/ws/pow?session=" + testSession2.getSessionId(),
        "8.8.4.4"
      );
      ...
      await awaitSleepPromise(
        500,
        () => fakeSocket2.getSentMessage("verify").length > 0
      );
      let verifyMsg = fakeSocket2.getSentMessage("verify");
      expect(verifyMsg.length).to.equal(
        1,
        "unexpected number of verify messages sent"
      );
      expect(verifyMsg[0].data.preimage).to.equal(
        "oXwNMIuRUOc=",
        "invalid verify message: preimage missmatch"
      );
      expect(verifyMsg[0].data.nonces[0]).to.equal(
        1524,
        "invalid verify message: nonces missmatch"
      );
      // send verify result
      fakeSocket2.emit(
        "message",
      ...
      await awaitSleepPromise(
        500,
        () => fakeSocket2.getSentMessage("updateBalance").length > 0
      );
      ...
      expect(balanceMsg1[0].data.balance).to.equal(
        "105",
        "invalid updateBalance message: unexpected balance"
      );
      expect(balanceMsg1[0].data.reason).to.matches(
        /valid verification/,
        "invalid updateBalance message: unexpected reason"
      );
      await awaitSleepPromise(
        500,
        () => fakeSocket1.getSentMessage("ok").length > 0
      );
