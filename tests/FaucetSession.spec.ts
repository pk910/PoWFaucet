...
describe("Faucet Session Management", () => {
  ...
  it("Create normal session", async () => {
    ...
    expect(testSession.getRemoteIP()).to.equal(
      "8.8.8.8",
      "unexpected remoteIP"
    );
    expect(testSession.getTargetAddr()).to.equal(
      "0x0000000000000000000000000000000000001337",
      "unexpected targetAddr"
    );
    expect(Math.abs(testSession.getStartTime() - now)).to.be.lessThan(
      2,
      "unexpected startTime"
    );
    expect(testSession.getBlockingTasks().length).to.equal(
      0,
      "unexpected blockingTasks"
    );
    ...
    expect(testSession.getDropAmount()).to.equal(
      BigInt(faucetConfig.maxDropAmount),
  ...
  it("Create session with blocking task", async () => {
    ...
    expect(testSession.getDropAmount()).to.equal(
      1337n,
      "unexpected drop amount after addReward()"
    );
    await testSession.subPenalty(10n);
    expect(testSession.getDropAmount()).to.equal(
      1327n,
      "unexpected drop amount after subPenalty()"
    );
    let runningSession = sessionManager.getSession(testSession.getSessionId(), [
      FaucetSessionStatus.RUNNING,
    ]);
    expect(runningSession === testSession).to.equal(
      true,
      "sessionManager.getSession did not return running session (running state)"
    );
    let runningSession2 = sessionManager.getSession(testSession.getSessionId());
    expect(runningSession2 === testSession).to.equal(
      true,
      "sessionManager.getSession did not return running session (stateless)"
    );
    await awaitSleepPromise(
      4000,
      () => testSession.getSessionStatus() === FaucetSessionStatus.CLAIMABLE
    );
    expect(testSession.getSessionStatus()).to.equal(
    ...
    expect(testSession.getDropAmount()).to.equal(
      1327n,
      "unexpected drop amount after setDropAmount()"
    );
  }).timeout(5000);
  ...
  it("Create invalid session (amount too low)", async () => {
    ...
    expect(testSession.getSessionData("failed.code")).to.equal(
      "AMOUNT_TOO_LOW",
  ...
  it("Restore valid session", async () => {
    ...
    let testSession = sessionManager.getSession(
      "4e63566e-e482-46f3-bb91-da11f511aae5",
      [FaucetSessionStatus.RUNNING]
    );
    ...
    expect(testSession.getSessionModuleRef("test.info")).to.equal(
      "info1234",
      "unexpected getSessionModuleRef result"
    );
  });
  ...
  it("Restore invalid session (timed out)", async () => {
    ...
    await sessionManager.initialize();
    let testSession = sessionManager.getSession(
      "4e63566e-e482-46f3-bb91-da11f511aae6",
      [FaucetSessionStatus.RUNNING]
    );
    expect(testSession).to.not.equal(undefined, "getSession failed");
    await testSession.tryProceedSession();
    let sessionData = await sessionManager.getSessionData(
      "4e63566e-e482-46f3-bb91-da11f511aae6"
    );
    ...
    expect(sessionData.data["failed.code"]).to.equal(
      "SESSION_TIMEOUT",
      "unexpected error code"
    );
  });
  ...
  it("Check session task handling ", async () => {
    ...
    expect(error && error instanceof FaucetError).to.equal(
      true,
      "unexpected error type"
    );
    ...
    expect(testSession.getBlockingTasks().length).to.equal(
      1,
      "unexpected blockingTasks count after resolving first task"
    );
    testSession.resolveBlockingTask("test", "test2");
    expect(testSession.getBlockingTasks().length).to.equal(
      0,
      "unexpected blockingTasks count after resolving second task"
    );
    await awaitSleepPromise(
      4000,
      () => testSession.getSessionStatus() === FaucetSessionStatus.CLAIMABLE
  ...
  it("Check invalid session property changes", async () => {
    ...
    expect(testSession.getDropAmount()).to.equal(
      0n,
      "unexpected drop amount after subPenalty from initial balance"
    );
    ...
    expect(testSession.getDropAmount()).to.equal(
      50n,
      "unexpected drop amount after setDropAmount on failed session"
    );
    await testSession.addReward(1000n);
    expect(testSession.getDropAmount()).to.equal(
      50n,
      "unexpected drop amount after addReward on failed session"
    );
    await testSession.subPenalty(1000n);
    expect(testSession.getDropAmount()).to.equal(
      50n,
      "unexpected drop amount after subPenalty on failed session"
    );
    let sessionInfo = await testSession.getSessionInfo();
    expect(sessionInfo.session).to.equal(
      testSession.getSessionId(),
      "invalid sessioninfo: id missmatch"
    );
    expect(sessionInfo.balance).to.equal(
      testSession.getDropAmount().toString(),
      "invalid sessioninfo: balance missmatch"
    );
    expect(sessionInfo.failedCode).to.equal(
      "AMOUNT_TOO_LOW",
      "invalid sessioninfo: failedCode missmatch"
    );
  });
  ...
  it("Check SessionManager: get session data", async () => {
    ...
    expect(testSession).to.not.equal(null, "createSession failed");
    expect(
      sessionManager.getSession(testSession.getSessionId(), [
        FaucetSessionStatus.UNKNOWN,
      ])
    ).to.equal(null, "unexpected getSession result for non-matching state");
    expect(
      sessionManager.getSession("4e63566e-e482-46f3-bb91-da11f511aae0", [
        FaucetSessionStatus.UNKNOWN,
      ])
    ).to.equal(undefined, "unexpected getSession result for unknown session");
    expect(
      await sessionManager.getSessionData(testSession.getSessionId())
    ).to.not.equal(null, "unexpected getSessionData result for known session");
    expect(
      await sessionManager.getSessionData(
        "4e63566e-e482-46f3-bb91-da11f511aae0"
      )
    ).to.equal(null, "unexpected getSessionData result for unknown session");
    expect(sessionManager.getActiveSessions().length).to.equal(
      1,
      "unexpected getActiveSessions result count"
    );
  });
  ...
  it("Check SessionManager: getUnclaimedBalance", async () => {
    ...
    await testSession.addReward(1000n);
    expect(await sessionManager.getUnclaimedBalance()).to.equal(
      1000n,
      "unexpected getUnclaimedBalance result"
    );
  });
  ...
  it("Check SessionManager: session timeout processing", async () => {
    ...
    let session1 = await sessionManager.getSessionData(
      "4e63566e-e482-46f3-bb91-da11f511ab01"
    );
    expect(session1).to.not.equal(null, "getSessionData failed");
    expect(session1.status).to.equal(
      FaucetSessionStatus.FAILED,
      "unexpected session status"
    );
    expect(session1.data["failed.code"]).to.equal(
      "SESSION_TIMEOUT",
      "unexpected error code"
    );
    let session2 = await sessionManager.getSessionData(
      "4e63566e-e482-46f3-bb91-da11f511ab02"
    );
    expect(session2).to.not.equal(null, "getSessionData failed");
    expect(session2.status).to.equal(
      FaucetSessionStatus.FAILED,
      "unexpected session status"
    );
    expect(session2.data["failed.code"]).to.equal(
      "SESSION_TIMEOUT",
      "unexpected error code"
    );
    let testSession = await sessionManager.createSession("::ffff:8.8.8.8", {
