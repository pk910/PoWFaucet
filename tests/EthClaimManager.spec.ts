...
import {
  ...
  EthClaimInfo,
  EthClaimManager,
} from "../src/eth/EthClaimManager.js";
...
describe("ETH Claim Manager", () => {
  ...
  beforeEach(async () => {
    globalStubs = bindTestStubs({
      "EthWalletManager.sendClaimTx": sinon.stub(
        EthWalletManager.prototype,
        "sendClaimTx"
      ),
    });
    loadDefaultTestConfig();
  ...
  it("Load stored claim queue", async () => {
    ...
    await addTestSession(FaucetSessionStatus.CLAIMING, {
      ...
      claimTime: Math.floor(new Date().getTime() / 1000),
      txHash:
        "0xdb5950d44ceed2a5eb77970104b974b8c4234d7110fd8d0008edb2cfff835f04",
    });
    await addTestSession(FaucetSessionStatus.CLAIMING, {
      claimIdx: 2,
      ...
      claimTime: Math.floor(new Date().getTime() / 1000),
      txHash:
        "0x7c4074c4d182e312adf698795835cbd2d597246cdd47013f09c7bb81f33a363c",
    });
    ...
    globalStubs["watchClaimTx"] = sinon
      .stub(EthWalletManager.prototype, "watchClaimTx")
      .returns(new Promise(() => {}));

    let claimManager = ServiceManager.GetService(EthClaimManager);
    await claimManager.initialize();

    expect(claimManager.getQueuedAmount()).to.equal(
      200n,
      "unexpected queued balance"
    );
    let txQueue = claimManager.getTransactionQueue(true);
    expect(txQueue.length).to.equal(2, "unexpected queue length");
    expect(txQueue[0].session).to.equal(
      ses4.sessionId,
      "unexpected queue order"
    );
  });
  ...
  it("Check restored pending claim processing", async () => {
    let ses1 = await addTestSession(FaucetSessionStatus.CLAIMING, {
      ...
      claimTime: Math.floor(new Date().getTime() / 1000),
      txHash:
        "0x7c4074c4d182e312adf698795835cbd2d597246cdd47013f09c7bb81f33a3601",
    });
    let ses2 = await addTestSession(FaucetSessionStatus.CLAIMING, {
      ...
      claimTime: Math.floor(new Date().getTime() / 1000),
      txHash:
        "0x7c4074c4d182e312adf698795835cbd2d597246cdd47013f09c7bb81f33a3602",
    });
    let ses3 = await addTestSession(FaucetSessionStatus.CLAIMING, {
      ...
      claimTime: Math.floor(new Date().getTime() / 1000),
      txHash:
        "0x7c4074c4d182e312adf698795835cbd2d597246cdd47013f09c7bb81f33a3603",
    });

    let txResFuse: any = {};
    let txResults: {
      [sesId: string]: Promise<{
        status: boolean;
        block: number;
        fee: bigint;
        receipt: any;
    ...
    globalStubs["watchClaimTx"] = sinon
      .stub(EthWalletManager.prototype, "watchClaimTx")
      .callsFake((claimTx) => txResults[claimTx.session]);
    ...
    expect(claimed[ses1.sessionId]?.claim.claimStatus).to.equal(
      ClaimTxStatus.CONFIRMED,
      "unexpected ses1 claim status"
    );

    txResFuse.ses2 = true;
    await awaitSleepPromise(500, () => !!claimed[ses2.sessionId]);
    expect(claimed[ses2.sessionId]?.claim.claimStatus).to.equal(
      ClaimTxStatus.CONFIRMED,
      "unexpected ses2 claim status"
    );

    txResFuse.ses3 = true;
    let claim3 = claimManager
      .getTransactionQueue()
      .filter((t) => t.session === ses3.sessionId)[0];
    await awaitSleepPromise(
      500,
      () => claim3?.claim.claimStatus === ClaimTxStatus.FAILED
    );
    expect(claim3?.claim.claimStatus).to.equal(
      ClaimTxStatus.FAILED,
      "unexpected ses3 claim status"
    );
  });
  ...
  it("Create session claim, Invalid: session not claimable", async () => {
    ...
    expect(claimManager.getTransactionQueue().length).to.equal(
      0,
      "unexpected queue count"
    );
  });
  ...
  it("Create session claim: Trim too high amount", async () => {
    ...
    let sessionData = await ServiceManager.GetService(
      SessionManager
    ).getSessionData(testSession.sessionId);
    expect(sessionData.status).to.equal(
      FaucetSessionStatus.CLAIMING,
      "unexpected session status"
    );
    expect(sessionData.dropAmount).to.equal(
      "50",
      "unexpected session drop amount"
    );
  });
  ...
  it("Queue processing: Check processing if wallet not ready (skip)", async () => {
    ...
    expect(claim.claim.claimStatus).to.equal(
      ClaimTxStatus.QUEUE,
      "unexpected claim status before processQueue"
    );

    globalStubs["getWalletState"] = sinon
      .stub(EthWalletManager.prototype, "getWalletState")
      .returns({
        ready: false,
        nonce: 0,
        balance: 0n,
    ...
    expect(claim.claim.claimStatus).to.equal(
      ClaimTxStatus.QUEUE,
      "unexpected claim status after processQueue"
    );
  });
  ...
  it("Queue processing: Check processing if wallet not ready (fail)", async () => {
    ...
    expect(claim.claim.txError).to.matches(
      /RPC is currently unreachable/,
      "unexpected claim error message"
    );
  });
  ...
  it("Queue processing: Check processing if transaction fails unexpectedly (fail)", async () => {
    ...
    globalStubs["EthWalletManager.sendClaimTx"].returns(
  ...
  it("Queue processing: Check processing if transaction creation fails unexpectedly", async () => {
    ...
    expect(claim.claim.claimStatus).to.equal(
      ...
      "unexpected claim status during processQueue"
    );
  ...
  it("Queue processing: Check processing if transaction processing fails unexpectedly", async () => {
    ...
    globalStubs["EthWalletManager.sendClaimTx"].returns(
      Promise.resolve({
        txHash:
          "0xdb5950d44ceed2a5eb77970104b974b8c4234d7110fd8d0008edb2cfff835f01",
    ...
    await awaitSleepPromise(
      100,
      () => claim.claim.claimStatus === ClaimTxStatus.FAILED
    );
    expect(claim.claim.claimStatus).to.equal(
      ClaimTxStatus.FAILED,
      "unexpected claim status after tx failure"
    );
    expect(claim.claim.txError).to.matches(
      /test error/,
      "unexpected claim error message"
  ...
  it("Queue processing: Check processing for successful transaction", async () => {
    ...
    await awaitSleepPromise(
      1000,
      () => claim.claim.claimStatus === ClaimTxStatus.CONFIRMED
    );

    expect(claim.claim.claimStatus).to.equal(
      ClaimTxStatus.CONFIRMED,
      "unexpected claim status after tx confirmation"
    );
  });
  ...
  it("Notification Websocket: Check initialization (invalid url)", async () => {
    ...
    let fakeWs1 = await injectFakeWebSocket(
      "/ws/claim?session=session=[]12&&session=false",
      "8.8.8.8"
    );
    let errorMsg = fakeWs1.getSentMessage("error");
    expect(errorMsg.length).to.equal(1, "no error message returned");
    expect(errorMsg[0].data.reason).to.matches(
      /session not found/,
  ...
  it("Notification Websocket: Check claim notifications", async () => {
    ...
    let fakeWs2 = await injectFakeWebSocket(
      "/ws/claim?session=" + testSession2.sessionId,
      "8.8.8.8"
    );
    expect(fakeWs2.isReady).to.equal(true, "websocket2 was closed");

    await claimManager.processQueue();
    expect(claim1.claim.claimStatus).to.equal(
      ClaimTxStatus.PENDING,
      "unexpected claim1 status after processQueue"
    );
    expect(claim2.claim.claimStatus).to.equal(
      ClaimTxStatus.PENDING,
      "unexpected claim2 status after processQueue"
    );

    let fakeWs1 = await injectFakeWebSocket(
      "/ws/claim?session=" + testSession1.sessionId,
      "8.8.8.8"
    );
    expect(fakeWs1.isReady).to.equal(true, "websocket1 was closed");
    await awaitSleepPromise(
      100,
      () => fakeWs1.getSentMessage("update").length > 1
    );
    let updateMsg = fakeWs1.getSentMessage("update");
    expect(updateMsg.length).to.equal(1, "no update message sent");
    expect(updateMsg[updateMsg.length - 1].data.processedIdx).to.equal(
      2,
      "unexpected processed count in last update"
    );
    ...
    await awaitSleepPromise(
      100,
      () => claim1.claim.claimStatus === ClaimTxStatus.CONFIRMED
    );
    await claimManager.processQueue();
    await awaitSleepPromise(
      100,
      () => fakeWs2.getSentMessage("update").length > 2
    );

    updateMsg = fakeWs2.getSentMessage("update");
    expect(updateMsg.length).to.equal(
      2,
      "no update message sent on 1st confirmation"
    );
    expect(updateMsg[updateMsg.length - 1].data.processedIdx).to.equal(
      2,
      "unexpected processed count in last update"
    );
    expect(updateMsg[updateMsg.length - 1].data.confirmedIdx).to.equal(
      1,
      "unexpected confirmed count in last update"
    );

    claimFuses[1]();
    await awaitSleepPromise(
      100,
      () => claim2.claim.claimStatus === ClaimTxStatus.CONFIRMED
    );
    await claimManager.processQueue();
    await awaitSleepPromise(
      100,
      () => fakeWs2.getSentMessage("update").length > 3
    );

    updateMsg = fakeWs2.getSentMessage("update");
    expect(updateMsg.length).to.equal(
      3,
      "no update message sent on 2nd confirmation"
    );
    expect(updateMsg[updateMsg.length - 1].data.processedIdx).to.equal(
      2,
      "unexpected processed count in last update"
    ...
    expect(errorMsg.length).to.equal(
      1,
      "no error message sent after confirmation"
    );
  });
