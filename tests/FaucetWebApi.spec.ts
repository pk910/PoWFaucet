...
describe("Faucet Web API", () => {
  ...
  async function addTestSession(
    data: Partial<FaucetSessionStoreData>
  ...
  it("check unknown endpoint call", async () => {
    ...
    let apiResponse = await webApi.onApiRequest(
      encodeApiRequest({
        url: "/api/unknown_Endpoint_126368",
        remoteAddr: "8.8.8.8",
      })
    );
    expect(apiResponse instanceof FaucetHttpResponse).equal(
      true,
      "no api error response"
    );
  ...
  it("check /api/startSession", async () => {
    ...
    expect(!!apiResponse).equal(true, "no api response");
    expect(apiResponse.status).equal(
      "claimable",
      "unexpected response session status"
    );
  });
  ...
  it("check /api/startSession (invalid method)", async () => {
    ...
    expect(apiResponse.reason).equal(
      "Method Not Allowed",
      "unexpected response http reason"
    );
  });
  ...
  it("check /api/startSession (invalid input data)", async () => {
    ...
    expect(!!apiResponse).equal(true, "no api response");
    expect(apiResponse.failedCode).equal(
      "INVALID_ADDR",
  ...
  it("check /api/getSession", async () => {
    ...
    expect(apiResponse.status).equal(
      "running",
      "invalid response: unexpected session status"
    );
    expect(apiResponse.session).equal(
      testSession.getSessionId(),
      "invalid response: unexpected session id"
    );
    expect(apiResponse.target).equal(
      "0x0000000000000000000000000000000000001337",
      "invalid response: unexpected target addr"
    );
  });
  ...
  it("check /api/getSession (unknown session)", async () => {
    ...
    expect(!!apiResponse).equal(true, "no api response");
    expect(apiResponse.error).equal(
      "Session not found",
  ...
  it("check /api/getSession (unexpected error)", async () => {
    ...
    expect(apiResponse.failedReason).matches(
      /test error/,
  ...
  it("check /api/getSessionStatus (running session)", async () => {
    ...
    expect(!!apiResponse.details).equal(
      true,
      "invalid response: missing details"
    );
  });
  ...
  it("check /api/getSessionStatus (claiming session)", async () => {
    ...
    expect(apiResponse.claimStatus).equal(
      "queue",
      "invalid response: unexpected claim status"
    );
  });
  ...
  it("check /api/getSessionStatus (failed session)", async () => {
    ...
    expect(apiResponse.failedCode).equal(
      "TEST_ERROR",
      "invalid response: unexpected failedCode"
    );
  });
  ...
  it("check /api/getFaucetStatus", async () => {
    ...
    expect(!!apiResponse).equal(true, "no api response");
    expect(apiResponse.status.unclaimedBalance).equal(
      "100",
      "value mismatch: unclaimedBalance"
    );
    expect(apiResponse.status.queuedBalance).equal(
      "0",
      "value mismatch: queuedBalance"
    );
    expect(apiResponse.sessions.length).equal(
      1,
      "value mismatch: sessions.length"
    );
    expect(apiResponse.sessions[0].id).equal(
      "705f5ce0baec58a47344",
      "value mismatch: session.id"
    );
    expect(apiResponse.sessions[0].start).equal(
      sessionTime,
      "value mismatch: session.start"
    );
    expect(apiResponse.sessions[0].target).equal(
      "0x0000000000000000000000000000000000001337",
      "value mismatch: session.target"
    );
    expect(apiResponse.sessions[0].ip).equal(
      "bce.2a7.116.9f9",
      "value mismatch: session.ip"
    );
    expect(apiResponse.sessions[0].balance).equal(
      "100",
      "value mismatch: session.balance"
    );
    expect(apiResponse.sessions[0].nonce).equal(
      42,
      "value mismatch: session.nonce"
    );
    expect(apiResponse.sessions[0].status).equal(
      "running",
      "value mismatch: session.status"
    );
  });
  ...
  it("check /api/getQueueStatus", async () => {
    ...
    expect(apiResponse.claims[0].status).equal(
      "queue",
      "unexpected claim status"
    );
    expect(apiResponse.claims[0].session).equal(
      "705f5ce0baec58a47344",
      "value mismatch: claim.session"
    );
    expect(apiResponse.claims[0].time).equal(
      claimTime,
      "value mismatch: claim.time"
    );
    expect(apiResponse.claims[0].target).equal(
      "0x0000000000000000000000000000000000001337",
      "value mismatch: claim.target"
    );
  });
});
