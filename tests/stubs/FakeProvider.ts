...
> extends Web3BaseProvider<API> {
  ...
  private getLastRequest(method: string): any {
    let methodCalls;
    if (!(methodCalls = this.requestDict[method])) return null;
  ...
  private getResponse(payload) {
    ...
    else this.requestDict[payload.method].push(payload);
    ...
    let rspStub;
    try {
      if (typeof rsp === "function") rsp = rsp(payload);
      rspStub = {
        jsonrpc: "2.0",
  ...
  >(
    ...
    let response;
    if (Array.isArray(payload)) response = this.getResponses(payload);
    else response = this.getResponse(payload);
