...
export class FaucetStatus {
  ...
  private resetUpdateTimer() {
    ...
    this.updateTimer = setInterval(
      () => this.updateLocalStatus(),
      (faucetConfig.faucetStatus?.refresh || 10) * 1000
    );
  }
  ...
  private updateLocalStatus() {
    ...
    let faucetStatusJsonFile = faucetConfig.faucetStatus?.json
      ? resolveRelativePath(faucetConfig.faucetStatus.json)
      : null;
    let faucetStatusYamlFile = faucetConfig.faucetStatus?.yaml
      ? resolveRelativePath(faucetConfig.faucetStatus.yaml)
      : null;
    let faucetStatusStr = "";
    ...
    if (faucetStatusJsonFile && fs.existsSync(faucetStatusJsonFile)) {
      try {
        ...
        if (typeof faucetStatusJson === "string")
          this.localStatusEntries.push({
            level: FaucetStatusLevel.INFO,
            text: faucetStatusJson,
            prio: 10,
          });
        else if (
          typeof faucetStatusJson === "object" &&
          faucetStatusJson &&
          faucetStatusJson.text
        )
          this.localStatusEntries.push(faucetStatusJson);
        else if (
          typeof faucetStatusJson === "object" &&
          Array.isArray(faucetStatusJson)
        )
          Array.prototype.push.apply(this.localStatusEntries, faucetStatusJson);
      } catch (ex) {
        ServiceManager.GetService(FaucetProcess).emitLog(
          FaucetLogLevel.WARNING,
          "cannot read local faucet statu from json: " + ex.toString()
        );
      }
    }
    ...
    if (faucetStatusYamlFile && fs.existsSync(faucetStatusYamlFile)) {
      try {
        ...
        if (typeof faucetStatusYaml === "string")
          this.localStatusEntries.push({
            level: FaucetStatusLevel.INFO,
            text: faucetStatusYaml,
            prio: 10,
          });
        else if (
          typeof faucetStatusYaml === "object" &&
          faucetStatusYaml &&
          faucetStatusYaml.text
        )
          this.localStatusEntries.push(faucetStatusYaml);
        else if (
          typeof faucetStatusYaml === "object" &&
          Array.isArray(faucetStatusYaml)
        )
          Array.prototype.push.apply(this.localStatusEntries, faucetStatusYaml);
      } catch (ex) {
        ServiceManager.GetService(FaucetProcess).emitLog(
          FaucetLogLevel.WARNING,
          "cannot read local faucet statu from yaml: " + ex.toString()
        );
      }
    }
  ...
  public setFaucetStatus(
    key: string,
    statusText: string,
    statusLevel: FaucetStatusLevel,
    prio?: number
  ): IFaucetStatusEntry {
    if (statusText) {
      if (
        !this.currentStatus[key] ||
        this.currentStatus[key].text !== statusText
      ) {
        this.currentStatus[key] = {
          key: key,
          text: statusText,
  ...
  private getCachedValue(key: string, timeout: number, getter: () => any): any {
    ...
    if (
      (cachedValue = this.statusValueCache[key]) &&
      cachedValue.time > now - timeout
    ) {
  ...
  public getFaucetStatus(
    clientVersion?: string,
    session?: FaucetSession
  ): { status: IFaucetStatus[]; hash: string } {
    ...
    let checkStatusFilter = (status: IFaucetStatusEntry) => {
      ...
      if (
        status.filter.session !== undefined &&
        status.filter.session !== !!session
      )
        return false;

      let ipinfo = session ? session.getSessionData("ipinfo.data") : null;
      if (status.filter.country !== undefined) {
        if (!ipinfo || !ipinfo.countryCode) return false;
        let countries = Array.isArray(status.filter.country)
          ? status.filter.country
          : [status.filter.country];
        if (countries.indexOf(ipinfo.countryCode) === -1) return false;
      }
      if (
        status.filter.hosting !== undefined &&
        (!ipinfo || !!ipinfo.hosting !== status.filter.hosting)
      )
        return false;
      if (
        status.filter.proxy !== undefined &&
        (!ipinfo || !!ipinfo.proxy !== status.filter.proxy)
      )
        return false;
      ...
      if (status.filter.gt_hashrate !== undefined) {
        let hashrate = this.getWellKnownValue("hashrate");
        if (hashrate < status.filter.gt_hashrate) return false;
      }
    ...
    for (let i = 0; i < this.localStatusEntries.length; i++) {
      ...
      if (!checkStatusFilter(status)) continue;
      let statusKey = status.key || "*";
      if (
        !localStatusDict.hasOwnProperty(statusKey) ||
        (status.prio || 0) > (localStatusDict[statusKey].prio || 0)
      )
        localStatusDict[statusKey] = status;
    }
    Object.values(localStatusDict)
    ...
    if (session) {
      ...
      if (restriction) {
        restriction.messages.forEach((message) => {
          ...
          addStatus({
            level:
              typeof message.notify === "string"
                ? (message.notify as FaucetStatusLevel)
