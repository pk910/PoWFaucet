...
export class FaucetHttpServer {
  ...
  public getListenPort(): number {
    ...
    if (typeof addr === "object") return addr.port;
    else return faucetConfig.serverPort;
  }

  public addWssEndpoint(
    key: string,
    pattern: RegExp,
    handler: (req: IncomingMessage, ws: WebSocket, remoteIp: string) => void
  ) {
    ...
    if (req.method === "GET" || req.method === "POST") {
      ...
      req.on("end", () => {
        if ((req.url + "").match(/^\/api\//i)) {
          ...
          ServiceManager.GetService(FaucetWebApi)
            .onApiRequest(req, body)
            .then((res: object) => {
              if (
                res &&
                typeof res === "object" &&
                res instanceof FaucetHttpResponse
              ) {
                rsp.writeHead(res.code, res.reason, {
                  ...res.headers,
                  ...CORS_HEADERS,
                });
                rsp.end(res.body);
            ...
            .catch((err) => {
              ...
              if (
                err &&
                typeof err === "object" &&
                err instanceof FaucetHttpResponse
              ) {
                rsp.writeHead(err.code, err.reason, {
                  ...err.headers,
                  ...CORS_HEADERS,
                });
                rsp.end(err.body);
          ...
          ServiceManager.GetService(FaucetWebApi)
            ...
            .then((res: object) => {
              if (
                ...
                const { contentType, metrics } = res as {
                  contentType: string;
                  metrics: string;
                } & Record<string, any>;
  ...
  private onHttpUpgrade(
    ...
    this.wssServer.handleUpgrade(req, socket, head, (ws) => {
      let remoteAddr =
        ServiceManager.GetService(FaucetWebApi).getRemoteAddr(req);
