...
import {FaucetSession, ISessionStartUserInput} from "../../session/FaucetSession.js";
...
export class RecurringLimitsModule extends BaseModule<IRecurringLimitsConfig> {
  ...
  protected override startModule(): Promise<void> {
    this.moduleManager.addActionHook(
      ...
      (session: FaucetSession, userInput: ISessionStartUserInput) => this.processSessionStart(session, userInput)
    );
    this.moduleManager.addActionHook(
  ...
  private async processSessionStart(session: FaucetSession, _userInput: ISessionStartUserInput): Promise<void> {
  ...
  private async checkLimit(session: FaucetSession, limit: IRecurringLimitConfig): Promise<void> {
    ...
    const finishedSessions = await ServiceManager.GetService(FaucetDatabase).getFinishedSessions({
      targetAddr: session.getTargetAddr(), remoteIP, userId
    },  limit.duration, true);
