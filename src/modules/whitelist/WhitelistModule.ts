...
import {
  defaultConfig,
  IWhitelistConfig,
  IWhitelistEntryConfig,
} from "./WhitelistConfig.js";
...
export class WhitelistModule extends BaseModule<IWhitelistConfig> {
  ...
  private cachedWhitelistEntries: [
    pattern: string,
    restriction: IWhitelistEntryConfig,
  ][];
  ...
  public refreshCachedWhitelistEntries(force?: boolean) {
    ...
    let refresh = this.moduleConfig.whitelistFile
      ? this.moduleConfig.whitelistFile.refresh
      : 30;
    if (this.cachedWhitelistRefresh > now - refresh && !force) return;
    ...
    if (
      this.moduleConfig.whitelistFile &&
      this.moduleConfig.whitelistFile.yaml
    ) {
      // load yaml file
      if (Array.isArray(this.moduleConfig.whitelistFile.yaml))
        this.moduleConfig.whitelistFile.yaml.forEach((file) =>
          this.refreshCachedWhitelistEntriesFromYaml(resolveRelativePath(file))
        );
      else
        this.refreshCachedWhitelistEntriesFromYaml(
          resolveRelativePath(this.moduleConfig.whitelistFile.yaml)
        );
    }
  }
  ...
  private getSessionWhitelistEntry(
    session: FaucetSession
  ): IWhitelistEntryConfig | null {
