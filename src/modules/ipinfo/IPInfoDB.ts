...
export class IPInfoDB extends FaucetModuleDB {
  ...
  public override async cleanStore(): Promise<void> {
    await this.db.run("DELETE FROM IPInfoCache WHERE Timeout < ?", [
      this.now(),
    ]);
  }
  ...
  public async setIPInfo(
    ...
    let now = this.now();
    let row = await this.db.get(
      "SELECT Timeout FROM IPInfoCache WHERE IP = ?",
      [ip.toLowerCase()]
    );
    ...
    if (row) {
      await this.db.run(
        "UPDATE IPInfoCache SET Json = ?, Timeout = ? WHERE IP = ?",
        [infoJson, timeout, ip.toLowerCase()]
      );
    } else {
      await this.db.run(
        "INSERT INTO IPInfoCache (IP, Json, Timeout) VALUES (?, ?, ?)",
        [ip.toLowerCase(), infoJson, timeout]
      );
    }
  }
}
