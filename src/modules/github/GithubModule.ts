...
export class GithubModule extends BaseModule<IGithubConfig> {
  ...
  protected override async startModule(): Promise<void> {
    this.githubDb = await ServiceManager.GetService(
      FaucetDatabase
    ).createModuleDb(GithubDB, this);
    ...
    ServiceManager.GetService(FaucetWebApi).registerApiEndpoint(
      ...
      (req: IncomingMessage, url: IFaucetApiUrl, body: Buffer) =>
        this.processGithubAuthCallback(req, url, body)
    );
  ...
  private async processSessionStart(
    ...
    try {
      githubInfo = await this.githubResolver.getGithubInfo(
        userInput.githubToken,
        infoOpts
      );
    } catch (ex) {
      ServiceManager.GetService(FaucetProcess).emitLog(
        FaucetLogLevel.WARNING,
        "Error while fetching github info: " + ex.toString()
      );
    }
    ...
    for (let i = 0; i < this.moduleConfig.checks.length; i++) {
      ...
      if (check.required && passed === false) {
        let errMsg: string;
        if (check.message) errMsg = check.message.replace("{0}", errmsg);
        else
          errMsg =
            "Your github account does not meet the minimum requirements: " +
            errmsg;
        throw new FaucetError("GITHUB_CHECK", errMsg);
      }
      if (
        passed !== false &&
        typeof check.rewardFactor === "number" &&
        (rewardFactor === null || check.rewardFactor > rewardFactor)
      ) {
    ...
    if (githubInfo) {
      await Promise.all(
        this.moduleConfig.restrictions.map((restriction) =>
          this.checkRestriction(githubInfo.uid, restriction)
        )
      );
    }
  }
  ...
  private async processSessionRewardFactor(
    ...
    if (typeof githubFactor !== "number") return;
    rewardFactors.push({
      factor: githubFactor,
  ...
  private async processGithubAuthCallback(
    req: IncomingMessage,
    url: IFaucetApiUrl,
    body: Buffer
  ): Promise<any> {
    ...
    if ((errorCode = url.query["error"] as string)) {
      ...
      authResult["errorMessage"] =
        "Unknown error in github oauth authentication flow.";
    }
  ...
  private async checkRestriction(
    ...
    let finishedSessions = await this.githubDb.getGithubSessions(
      githubUserId,
    ...
    if (
      restriction.limitCount > 0 &&
      finishedSessions.length >= restriction.limitCount
    ) {
      let errMsg =
        restriction.message ||
        [
