...
export class GithubDB extends FaucetModuleDB {
  ...
  public override async cleanStore(): Promise<void> {
    await this.db.run("DELETE FROM GithubCache WHERE Timeout < ?", [
      this.now(),
    ]);
    ...
    while (dataIdx < rows.length) {
      ...
      promises.push(
        this.db
          .run(
            "DELETE FROM GithubSessions WHERE SessionId IN (" +
              dataBatch.map((b) => "?").join(",") +
              ")",
  ...
  public async setGithubInfo(
    ...
    let now = this.now();
    let row = await this.db.get(
      "SELECT Timeout FROM GithubCache WHERE UserId = ?",
      [userId.toString()]
    );
    ...
    if (row) {
      await this.db.run(
        "UPDATE GithubCache SET Json = ?, Timeout = ? WHERE UserId = ?",
        [infoJson, timeout, userId.toString()]
      );
    } else {
      await this.db.run(
        "INSERT INTO GithubCache (UserId, Json, Timeout) VALUES (?, ?, ?)",
        [userId.toString(), infoJson, timeout]
      );
    }
  }

  public getGithubSessions(
    userId: number,
    duration: number,
    skipData?: boolean
    ...
    return this.faucetStore.selectSessionsSql(
      ...
      [userId.toString(), now - duration],
      skipData
    );
  }

  public async setGithubSession(
    sessionId: string,
    userId: number
  ): Promise<void> {
    await this.db.run(
      SQL.driverSql({
        [FaucetDbDriver.SQLITE]:
          "INSERT OR REPLACE INTO GithubSessions (SessionId,UserId) VALUES (?,?)",
        [FaucetDbDriver.MYSQL]:
          "REPLACE INTO GithubSessions (SessionId,UserId) VALUES (?,?)",
      }),
