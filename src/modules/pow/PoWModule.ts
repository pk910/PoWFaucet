...
export class PoWModule extends BaseModule<IPoWConfig> {
  ...
  protected override startModule(): Promise<void> {
    ...
    ServiceManager.GetService(FaucetHttpServer).addWssEndpoint(
      "pow",
      /^\/ws\/pow($|\?)/,
      (req, ws, ip) => this.processPoWClientWebSocket(req, ws, ip)
    );
    ...
    this.moduleManager.addActionHook(
      ...
      1,
      "mining state",
      async (session: FaucetSession, moduleState: any) =>
        this.processSessionInfo(session, moduleState)
    );
    this.moduleManager.addActionHook(
      this,
  ...
  private async processSessionInfo(
    ...
    if (
      ...
      return;
    if (session.getSessionStatus() !== FaucetSessionStatus.RUNNING) return;
  ...
  private async processSessionComplete(session: FaucetSession): Promise<void> {
    ...
    setTimeout(() => {
      ...
      if (session.getSessionStatus() === FaucetSessionStatus.FAILED)
        powSession.activeClient?.killClient(
          "session failed: [" +
            session.getSessionData("failed.code") +
            "] " +
            session.getSessionData("failed.reason")
        );
      else powSession.activeClient?.killClient("session closed");
    }, 100);
  }
  ...
  private async processPoWClientWebSocket(
    req: IncomingMessage,
    ws: WebSocket,
    remoteIp: string
  ): Promise<void> {
    ...
    let session = ServiceManager.GetService(SessionManager).getSession(
      sessionId,
    ...
    try {
      ...
      let errData =
        ex instanceof FaucetError
          ? { code: ex.getCode(), message: ex.message }
          : {
              code: "INTERNAL_ERROR",
              message: "Could not update session IP: " + ex.toString(),
            };
      ws.send(
        JSON.stringify({
          action: "error",
    ...
    ServiceManager.GetService(FaucetProcess).emitLog(
      FaucetLogLevel.INFO,
      "connected PoWClient: " + session.getSessionId()
    );
  ...
  public disposePoWClient(client: PoWClient, reason: string) {
    ServiceManager.GetService(FaucetProcess).emitLog(
      FaucetLogLevel.INFO,
      "closed PoWClient: " +
        client.getFaucetSession().getSessionId() +
        " (" +
        reason +
        ")"
    );
    ...
    if (this.powClients[client.getFaucetSession().getSessionId()] === client) {
      ...
      ServiceManager.GetService(FaucetProcess).emitLog(
        ...
        "disposePoWClient: client not in active clients list: " +
          client.getFaucetSession().getSessionId()
      );
    }
  }
  ...
  public getPoWParamsStr(): string {
    switch (this.moduleConfig.powHashAlgo) {
      ...
      case PoWHashAlgo.CRYPTONIGHT:
        return (
          PoWHashAlgo.CRYPTONIGHT.toString() +
          "|" +
  ...
  private resetSessionIdleTimer(session: PoWSession) {
    ...
    if (hasActiveClient && idleTimer) {
      ...
      !idleTimer &&
      session.idleTime &&
      this.moduleConfig.powIdleTimeout
    ) {
      let now = nowSeconds();
      ...
      session.idleTimer = setTimeout(() => {
        ServiceManager.GetService(FaucetProcess).emitLog(
          FaucetLogLevel.INFO,
          "session idle timeout: " + session.getFaucetSession().getSessionId()
        );
