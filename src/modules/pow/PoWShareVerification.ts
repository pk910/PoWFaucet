...
export class PoWShareVerification {
  ...
  public static processVerificationResult(
    shareId: string,
    verifier: string,
    isValid: boolean
  ): boolean {
    if (!this.verifyingShares[shareId]) return false;
    return this.verifyingShares[shareId].processVerificationResult(
      verifier,
      isValid
    );
  }
  ...
  public startVerification(): Promise<IPoWShareVerificationResult> {
    if (this.resultDfd) return this.resultDfd.promise;
    ...
    if (
      validatorSessions.length < powConfig.verifyMinerPeerCount &&
      powConfig.verifyLocalLowPeerPercent > verifyLocalPercent
    )
    ...
    if (
      this.verifyLocal &&
      this.module.getValidator().getValidationQueueLength() >=
        powConfig.verifyLocalMaxQueue
    )
    ...
    if (this.verifyLocal) {
      ...
      this.module
        .getValidator()
        .validateShare(this.shareId, this.nonces, this.session.preImage)
        .then((isValid) => {
          if (!isValid) this.isInvalid = true;
          this.completeVerification();
        });
    } else if (
      powConfig.verifyMinerPercent > 0 &&
      validatorSessions.length >= powConfig.verifyMinerPeerCount &&
      Math.floor(Math.random() * 100) < powConfig.verifyMinerPercent
    ) {
      ...
      for (let i = 0; i < this.verifyMinerCount; i++) {
        ...
        this.verifyMinerSessions.push(
          validatorSession.getFaucetSession().getSessionId()
        );
  ...
  private getVerifierSessions(): PoWSession[] {
    let powConfig = this.module.getModuleConfig();
    let minBalance =
      (BigInt(powConfig.powShareReward) *
        BigInt(powConfig.verifyMinerMissPenaltyPerc * 100)) /
      10000n;
  ...
  public processVerificationResult(
    verifier: string,
    isValid: boolean
  ): boolean {
    let validatorIdx = this.verifyMinerSessions.indexOf(verifier);
    if (validatorIdx === -1) return false;
  ...
  private completeVerification() {
    ...
    if (this.isInvalid && !this.verifyLocal) {
      ...
      this.module
        ...
        .validateShare(this.shareId, this.nonces, this.session.preImage)
        .then((isValid) => {
          if (isValid) this.isInvalid = false;
          this.completeVerification();
        });
      return;
    }
    ...
    if (this.verifyMinerSessions.length > 0) {
      ...
      this.verifyMinerSessions.forEach((verifierId) => {
        let verifierFaucetSession = ServiceManager.GetService(
          SessionManager
        ).getSession(verifierId, [FaucetSessionStatus.RUNNING]);
            ...
            verifierFaucetSession.subPenalty(missPenalty).then(() => {
              ...
              if ((client = session.activeClient)) {
                client.sendMessage("updateBalance", {
                  ...
                  reason:
                    "verify miss (penalty: " + missPenalty.toString() + ")",
                });
              }
            });
          }
    ...
    if (this.isInvalid) {
      ...
      shareReward = this.session
        .getFaucetSession()
        .addReward(BigInt(powConfig.powShareReward));
    }
