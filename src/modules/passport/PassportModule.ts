...
export class PassportModule extends BaseModule<IPassportConfig> {
  ...
  protected override async startModule(): Promise<void> {
    this.passportDb = await ServiceManager.GetService(
      FaucetDatabase
    ).createModuleDb(PassportDB, this);
    ...
    this.moduleManager.addActionHook(
      ...
      async (clientConfig: any) => {
        clientConfig[this.moduleName] = {
          ...
          manualVerification:
            this.moduleConfig.trustedIssuers &&
            this.moduleConfig.trustedIssuers.length > 0,
    ...
    ServiceManager.GetService(FaucetWebApi).registerApiEndpoint(
      "refreshPassport",
      (req: IncomingMessage, url: IFaucetApiUrl, body: Buffer) =>
        this.processPassportRefresh(req, url, body)
    );
    ServiceManager.GetService(FaucetWebApi).registerApiEndpoint(
      "getPassportInfo",
      (req: IncomingMessage, url: IFaucetApiUrl, body: Buffer) =>
        this.processGetPassportInfo(req, url, body)
    );
  }

  protected override async stopModule(): Promise<void> {
    this.passportDb.dispose();
    ServiceManager.GetService(FaucetWebApi).removeApiEndpoint(
      "refreshPassport"
    );
    ServiceManager.GetService(FaucetWebApi).removeApiEndpoint(
      "getPassportInfo"
  ...
  private async processSessionStart(session: FaucetSession): Promise<void> {
    ...
    session.setSessionData(
      "passport.score",
      this.passportResolver.getPassportScore(passportInfo)
    );
  }
  ...
  private async processSessionInfo(
    ...
    let passportBoost = passportInfo
      ? this.passportResolver.getPassportScore(passportInfo)
      : null;
    moduleState[this.moduleName] = passportBoost;
  }

  private processSessionRewardFactor(
    session: FaucetSession,
  ...
  private async processPassportRefresh(
    req: IncomingMessage,
    url: IFaucetApiUrl,
    body: Buffer
  ): Promise<any> {
    ...
    if (req.method === "POST") {
      // manual refresh
      let verifyResult = await this.passportResolver.verifyUserPassport(
      ...
      if (now - lastRefresh < this.moduleConfig.refreshCooldown) {
        return {
          ...
          error:
            "Passport has been refreshed recently. Please wait " +
            (lastRefresh + this.moduleConfig.refreshCooldown - now) +
            " sec",
          cooldown: lastRefresh + this.moduleConfig.refreshCooldown,
        };
      }

      passportInfo = await this.passportResolver.getPassport(
        session.getTargetAddr(),
        true
      );
    }
  ...
  private async processGetPassportInfo(
    req: IncomingMessage,
    url: IFaucetApiUrl,
    body: Buffer
  ): Promise<any> {
