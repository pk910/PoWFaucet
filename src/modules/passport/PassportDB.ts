...
export class PassportDB extends FaucetModuleDB {
  ...
  public async getPassportStamps(
    stampHashs: string[]
  ): Promise<{ [hash: string]: string }> {
    let sql =
      "SELECT StampHash, Address FROM PassportStamps WHERE StampHash IN (" +
      stampHashs.map(() => "?").join(",") +
      ") AND Timeout > ?";
    let args: any[] = [];
  ...
  public async updatePassportStamps(
    stampHashs: string[],
    address: string,
    duration?: number
  ): Promise<void> {
    if (stampHashs.length === 0) return;

    let now = this.now();
    let timeout = now + (typeof duration === "number" ? duration : 86400);

    let queryArgs: any[] = [];
    let queryRows = stampHashs
      .map((stampHash) => {
    ...
    let query = await this.db.run(
      SQL.driverSql({
        [FaucetDbDriver.SQLITE]:
          "INSERT OR REPLACE INTO PassportStamps (StampHash, Address, Timeout) VALUES " +
          queryRows,
        [FaucetDbDriver.MYSQL]:
          "REPLACE INTO PassportStamps (StampHash, Address, Timeout) VALUES " +
          queryRows,
      }),
      queryArgs
    );
