...
function copyIfFoundOrRemove(filename, dstpath, dstname) {
  if (!copyIfFound(filename, dstpath, dstname)) {
    ...
    if (fs.existsSync(dstfile)) fs.rmSync(dstfile);
  }
}
...
new Promise((resolve, reject) => {
  ...
  compiler.run((err, res) => {
    ...
    fs.writeFileSync(
      path.join(distDir, "webpack-stats.json"),
      JSON.stringify(
        res.toJson({
          assets: false,
          hash: true,
